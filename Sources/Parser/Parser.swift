...
final class DSLParser {
  ...
  private func parseStruct(_ lines: [Substring], _ startIndex: Int) -> (StructNode, Int) {
    ...
    var implementsInterface: [String] = []

    var index = startIndex + 1
    while index < lines.count && !lines[index].starts(with: "}") {
      let line = lines[index]
      if line.contains("implements") {
        implementsInterface = line.replacingOccurrences(of: "{", with: "").trimmingCharacters(
          in: .whitespacesAndNewlines
        ).split(separator: " ").dropFirst(1).map { String($0) }
      } else {
    ...
    let structNode = StructNode(
      name: String(structName), fields: fields,
      implements: implementsInterface.isEmpty ? nil : implementsInterface)
    return (structNode, index)
  }

  private func parseInterface(_ lines: [Substring], _ startIndex: Int) -> (InterfaceNode, Int) {
    let interfaceName = lines[startIndex].split(separator: " ")[1]
